...
import { send } from '../services/BusinessAPI.js'
...
class Order {
    ...
    userCreate = async (req, res, next) => {
        const order = await this.create(req, res, next, 'user')
        await send(order.dataValues)
    }

    guestCreate = async (req, res, next) => {
        const order = await this.create(req, res, next, 'guest')
        await send(order.dataValues)
    }
