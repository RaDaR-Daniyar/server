import CatalogModel from '../models/Admin.js'
...
class Catalog {
    async getAll(req, res, next) {
        try {
            ...
            const products = await CatalogModel.getAll(options);
            res.json(products);
        } catch (e) {
    ...
    async getOne(req, res, next) {
        try {
            ...
            const product = await CatalogModel.getOne(req.params.id);
            res.json(product);
        } catch (e) {
            next(AppError.badRequest(e.message));
        }
    ...
    async create(req, res, next) {
        try {
            ...
            const product = await CatalogModel.create(req.body, req.files?.image);
            res.json(product);
        } catch (e) {
            next(AppError.badRequest(e.message));
        }
    ...
    async update(req, res, next) {
        try {
            ...
            const product = await CatalogModel.update(req.params.id, req.body, req.files?.image);
            res.json(product);
        } catch (e) {
            next(AppError.badRequest(e.message));
        }
    ...
    async delete(req, res, next) {
        try {
            ...
            const product = await CatalogModel.delete(req.params.id);
            res.json(product);
        } catch (e) {
            next(AppError.badRequest(e.message));
        }
